...
PDPoint MaillageChunk::next()
{
 ...
 if ((grid[available[i].x][available[i].x]!=-1) || (available[i].attempts>100)) available.erase(available.begin()+i);
...
bool MaillageChunk::isFree(PDPoint p)
{
 ...
 int xEnd=gridX+checkDist; if (xEnd>gridSize-1) xEnd=gridSize-1;
 int yStart=gridY-checkDist; if (yStart<0) yStart=0;
 int yEnd=gridY+checkDist; if (yEnd>gridSize-1) yEnd=gridSize-1;
